...
class Progress:
    ...
    def _get_color_from_verbosity(
        self, verbosity: t.Optional[int]
    ...
    def _render(self, args: t.Tuple, marks: ParsedMarks) -> str:
        ...
        if frame and config.show_source:
            ...
            lineno_part = color_text(
                f'{lineno:<3}', AnsiColor.BLUE, AnsiStyle.DIM
            )
        ...
        if args:
            if (
                marks.verbosity is not None
            ):  # only color text if we have explicit verbosity
                text_color, text_style = self._get_color_from_verbosity(
                    marks.verbosity
                )
                for arg in args:
                    if text_color:
        ...
        if caller_info:
            line_content += (
                caller_info
                + ' '
                + color_text('|', AnsiColor.BRIGHT_BLACK)
                + ' '
            )
